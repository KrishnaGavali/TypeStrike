...
import { saveTypingResult } from "./saveTypingResult";
...
const TypingBox = () => {
  ...
  const handleSaveTypingResult = async () => {
    console.log("Saving typing result");
    const { netSpeed, typingAccuracy } = typingStats;
    const data = await saveTypingResult(
      user.email,
      netSpeed,
      typingAccuracy,
      testTime
    ...
    <>
                ...
                {!showStats && (
                  <div className="flex justify-between w-[90%] mb-5">
                    ...
                    <p className="w-52 text-3xl flex justify-center">
                      <span key={currentTime}>{currentTime}</span>
                    </p>
                  </div>
                )}
              </AnimatePresence>
                ...
                >
                  <AnimatePresence mode="wait">
                    {wordsArray.map((word, wordIndex) => (
                      <span
                        className="word mr-6 font-extralight"
                        key={wordIndex}
                        ref={wordsSpanRefsArray[wordIndex]}
                      >
                        {word.map((letter, letterIndex) => (
                          <span className="font-extralight" key={letterIndex}>
                            {letter}
                          </span>
                        ))}
                      </span>
      ...
      <div className=" flex justify-center items-center">
        <motion.button
          ...
          whileTap={{ scale: 0.75, rotate: 360 }}
        ...
        {showStats && (
          <Popover.Root>
            <Popover.Trigger className="z-50 ml-4">
              <motion.button className="z-50" whileTap={{ scale: 0.75 }}>
                <img
                  width="48"
                  height="48"
                  className="cursor-pointer"
